...
def get_dtree_features(X) :
	...
	#without feature_list populated: accuracy is 0.90693
	#with feature_list populated:
	feature_list = ["gun","shot", "shooting", "shoot", "murder", "police", "homicide","armed", 
	"rampage", "kill", "killer","killing", "shooter", "gunman", "evidence","suicide","reporter","law","enact","measures",
	"scene","dead at scene","illness","ill","terrorist"]
	for x in X :
		f = {} #(terrorist: 1.0), (illness: 1.0)
...
def get_features(X) : 
	features = []
	for x in X :  #X is list of articles
		f = {}
		...
		# 		f[word] = 0
		# 	f[word]+=1
		# print len(X) = number of articles
...
def train_dtree_classifier(X, y):
	clf = DecisionTreeClassifier(max_depth=3) #93 with 1, 94 with 5, 95 with 10
	clf.fit(X,y)
	return clf
...
def rule_based_classifier(data):
	...
	for label, text in data : 
		prediction = '0'
		#TODO added more keywords, saw how well they do alone and in combination
		if "gun" in text : prediction = '1'
		if "shooting" in text : prediction = '1'
		if "shot" or "shooting" or "shoot" in text : prediction = '1'
		if "murder" in text : prediction = '1'
		if "police" in text : prediction = '1'
		if "homicide" in text : prediction = '1'
		if "armed" in text : prediction = '1'
		if "rampage" in text : prediction = '1'
		if "killed" or "kill" in text : prediction = '1'
		if "shooter" in text : prediction = '1'
		if "gunman" in text : prediction = '1'
		if "death" or "dead" in text : prediction = '1'
		if "evidence" in text : prediction = '1'
		if "suicide" in text : prediction = '1'
		if "reporter" in text : prediction = '1'
		if "law" or "enact" or "measures" in text : prediction = '1'
		if "scene" or "dead at scene" in text : prediction = '1'
		if "illness" or "ill" or "terrorist" in text : prediction = '1'
		if "basketball" or "goal" or "game" or "points" in text : prediction = '0'
